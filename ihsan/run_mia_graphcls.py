...
#  Patch FedALA + FedALA-R loss_fn bug
...
REPO_ROOT = os.getcwd()
DATA_ROOT = os.path.join(REPO_ROOT, "data_table6_graphfl_a1_multi", "inst_01")



# Robust model/logits helpers
...
def collect_member_nonmember_loaders(trainer, prefer_nonmember="test"):
    clients = pick_clients(trainer)
    train_loaders, non_loaders = [], []
    for c in clients:
        tl = pick_client_loader(c, "train")
        nl = pick_client_loader(c, "test" if prefer_nonmember == "test" else "val")
        if tl is not None: train_loaders.append(tl)
        if nl is not None: non_loaders.append(nl)
    if len(train_loaders) == 0:
        raise AttributeError("No client train loaders found.")
    if len(non_loaders) == 0:
        raise AttributeError(f"No client {prefer_nonmember} loaders found.")
...
# MIA computations (same attackers as Subgraph-FL)
