...
class GraphClsTask(BaseTask):
    ...
    def load_train_val_test_split(self):
        ...
        # ---- DEBUG SPLIT SIZES ----
        n_train = int(self.train_mask.sum().item())
        n_val = int(self.val_mask.sum().item())
        n_test = int(self.test_mask.sum().item())
        print(f"[client {self.client_id}] N={len(self.data)} train={n_train} val={n_val} test={n_test}")

        # ---- DEBUG LABEL DISTRIBUTION (train split) ----
        try:
            y_train = self.data.y[self.train_mask].detach().cpu().numpy()
            if y_train.size > 0:
                import numpy as np
                print(f"[client {self.client_id}] train label counts:", np.bincount(y_train))
        except Exception as e:
            print(f"[client {self.client_id}] label debug failed: {e}")
    ...
    def local_graph_train_val_test_split(self, local_graphs, split, shuffle=True):
        ...
        # Ensure non-empty val/test if possible (move samples from train)
        all_train_idx = train_mask.nonzero(as_tuple=False).view(-1).tolist()

        if val_mask.sum().item() == 0 and len(all_train_idx) >= 1:
            idx = all_train_idx.pop()  # take one from train
            train_mask[idx] = False
            val_mask[idx] = True

        if test_mask.sum().item() == 0 and len(all_train_idx) >= 1:
            idx = all_train_idx.pop()
            train_mask[idx] = False
            test_mask[idx] = True
